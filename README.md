...
**Automated Column-Description Generation — Fine-Tuning Phi-4**
...
1. `microsoft/phi-4` is an open-weights, **MIT-licensed and ungated** checkpoint, so no `HF_TOKEN` is required — set one (read scope) only if you point `MODEL_ID` at a gated checkpoint or private mirror, either exported in the environment or in `.env` (the GPU notebooks read `HF_TOKEN=` from `.env` in the cwd or `PROJECT_DIR`). `.env` is gitignored.
...
3. Run `finetune_descriptions.ipynb` → `splits.json`, SFT/DPO data + `sft_data/test_examples.jsonl`, and `adapters/phi-4-coldesc-dpo/`.
